...
const char* VERSION = "0.19";  // Version du programme
 /** @date 29/06/2025
 * 
...
// Les adresses EEPROM sont maintenant définies dans EEPROM_Manager.h
...
//Initialisatrion des varibles de l'EEPROM//
  LoadEEPROM();  // Charger les données de calibration depuis l'EEPROM
...
//Interface utilisateur
  ...
  //Serial.println("Systeme demarre - Calibration chargee depuis EEPROM");
  // DisplayEEPROMStatus(); // Suppression de l'affichage du statut EEPROM au démarrage
}
...
void PPS_Calibration () {//le mode calibration ne peux s'appliqué que si le moteur et a 0 RPM
 ...
 if (1==1){//currentStatus_RPM  == 0 && currentStatus_battery10 <= 12 && ActiveCalibratePPS == 1 ){ //Securité pour calibration
   ...
   while (millis() - currentMillis < 5000) {       //pendant x secondes
     ...
     //Calibration terminee - Sauvegarde via menu "Save"
     Serial.println ("PPS calibrated - Utilisez le menu 1.1.3 pour sauvegarder en EEPROM") ;
...
// Les fonctions EEPROM sont maintenant dans EEPROM_Manager.cpp
