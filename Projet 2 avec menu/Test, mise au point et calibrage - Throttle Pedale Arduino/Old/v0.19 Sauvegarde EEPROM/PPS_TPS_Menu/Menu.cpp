...
void afficherSousMenu(String menu) {
  ...
  if (menu == MENU_PEDALE) {  
    ...
    //!1.1.3
          ...
          digitalWrite(13, LOW);// Test d'appel d'une fonction (TO DO metre la fonction a la place))
  ...
  //!1.3 
    ...
    Serial.println("3) Save PWM en EEPROM");
    Serial.println("");
  ...
  //!1.4 
    ...
    Serial.println("2) Save Frequence en EEPROM");
    Serial.println("");
  ...
  //!1.5 
    ...
    //!1.5.1 
      ...
      Serial.println("4) Save PID Normal en EEPROM");
      Serial.println("");
      Serial.println("");
      Serial.println("");
      Serial.println("");
    ...
    //!1.5.2
      ...
      Serial.println("4) Save PID Agressif en EEPROM");
      Serial.println("");
      Serial.println("");
      Serial.println("");
      Serial.println("");
  ...
  //!1.6
    ...
    Serial.println("1) Read sensors");
    Serial.println("2) Actuator testing");
    Serial.println("3) DSG");
    Serial.println("4) Connexion");
    Serial.println("5) EEPROM");
    Serial.println("");
    Serial.println("");
    Serial.println("");
    Serial.println("");
    ...
    //!1.6.3
    else if (menu == MENU_DSG) {
        Serial.println("\n//" + niveauMenu + " DSG// ");
        Serial.println("1) Mode DSG");
        Serial.println("2) Durée séquence DSG");
        Serial.println("3) Save DSG en EEPROM");
        Serial.println("");
        Serial.println("");
       ...
       //!1.6.3.1
       else if (menu == MENU_DSG_Mode) {
           Serial.println("\n//" + niveauMenu + " Mode DSG// ");
           Serial.println("1) On"); //Finir le code pour valider le choix
    ...
    //!1.6.5
    else if (menu == MENU_ACQ_EEPROM) {
      Serial.println("\n//" + niveauMenu + " Menu EEPROM //");
      Serial.println("1) Save All Data");
      Serial.println("2) Load All Data");
      Serial.println("3) EEPROM Status");
...
void gererMenu(String option) {
  // Commandes globales EEPROM (peuvent être utilisées depuis n'importe quel menu)
  if (option == "save_all") {
  ...
  } else if (niveauMenu == MENU_DSG) {
    handleDSGMenu(option);
  } else if (niveauMenu == MENU_ACQ_EEPROM) {
    handleEEPROMMenu(option);
  } else {
...
// 1.3.1 Nouvelles fonctions pour gérer les sous-menus spécifiques
void handlePWMMenu(String option) {
  if (option == "1") {
    Serial.println("\n//" + niveauMenu + ".1 New value PWM Min// ");
    ...
    Serial.println("\n//" + niveauMenu + ".2 New value PWM Max// ");
    ...
    Serial.println("Menu 1.3.3 - Reglage PWM");
    Serial.println("Entrez une nouvelle valeur ou commande:");
    Serial.println("3 - Sauvegarder PWM en EEPROM");
    Serial.println("q) Quitter");
  }
...
void handleFrequencyMenu(String option) {
  if (option == "1") {
    ...
    Serial.println("Menu 1.4.1 - Reglage Frequence");
    Serial.println("Entrez une nouvelle valeur ou commande:");
    Serial.println("2 - Sauvegarder Frequence en EEPROM");
    Serial.println("q) Quitter");
  }
  else if (option.equalsIgnoreCase("q")) {
    niveauMenu = MENU_PRINCIPAL;
...
void handleNormalPIDMenu(String option) {
  if (option == "1") {
    ...
    Serial.println("Entrez une nouvelle valeur ou commande:");
    Serial.println("4 - Sauvegarder PID Normal en EEPROM");
    Serial.println("q) Quitter");
  }
  else if (option.equalsIgnoreCase("q")) {
    niveauMenu = MENU_REGLAGE_PID;
...
void handleAgressifPIDMenu(String option) {
  if (option == "1") {
    ...
    Serial.println("Entrez une nouvelle valeur ou commande:");
    Serial.println("4 - Sauvegarder PID Agressif en EEPROM");
    Serial.println("q) Quitter");
  }
  else if (option.equalsIgnoreCase("q")) {
    niveauMenu = MENU_REGLAGE_PID;
...
void handleDSGMenu(String option) {
  if (option == "save") {
    SaveEEPROM_DSG();
    Serial.println("Parametres DSG sauvegardes en EEPROM");
    Serial.println("Menu 1.6.3 - DSG");
    Serial.println("Entrez une nouvelle valeur ou commande:");
    ...
    niveauMenu = MENU_ACQUISITION;
    afficherSousMenu(MENU_ACQUISITION);
  }
}

void handleEEPROMMenu(String option) {
  if (option == "1") {
    SaveEEPROM();  // Sauvegarde toutes les données
    Serial.println("Toutes les donnees sauvegardees en EEPROM");
    Serial.println("Appuyez sur une touche pour continuer...");
    while (Serial.available() <= 0) {
      // Attendre l'entrée utilisateur
    }
    Serial.read(); // Vider le buffer
    afficherSousMenu(MENU_ACQ_EEPROM);
  }
  else if (option == "2") {
    LoadEEPROM();  // Recharge toutes les données
    Serial.println("Toutes les donnees rechargees depuis EEPROM");
    ...
    Serial.println("ATTENTION: Reinitialisation de l'EEPROM dans 3 secondes...");
    Serial.println("Appuyez sur 'Q' pour annuler...");
    delay(3000);
